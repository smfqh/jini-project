...
def start_second_dream():
    ...
    try: 
        ...
        while True:
            ...
            if start_time < now < end_time - datetime.timedelta(minutes=120):
                ...
                if Decimal(str(available_amt))  >=  Decimal(str(item_buy_amt)) :
                    ...
                    for target_item in target_items:
                        ...
                        logging.info('4. 현구매 수익율 :' + str(rev_pcnt))
                        logging.info('5. 목   표   가  :' + str(targetPrice))
                        logging.info('6. 목표가 수익율 :' + str(pre_pcnt))
                        logging.info("*********************************************************")
                    ...
                    for my_item in my_items:
                        ...
                        for ticker in tickers:
                            ...
                            if my_item['market'] == ticker['market']:
                                ...
                                if Decimal(str(rev_pcnt)) >= Decimal(str(my_pect))  and Decimal(str(targetPrice)) <= Decimal(str(current_price)) and round(item_income, 2) < 0: 
                                # -----------------------------------------------------------------
                                # 추매 
                                # -----------------------------------------------------------------                              
                                    available_amt = get_krwbal()['available_krw']                                
                    ...
                    for my_item in my_items:
                        ...
                        for ticker in tickers:
                            ...
                            if my_item['market'] == ticker['market']:
                                ...
                                current_income = (current_price-prev_close_price) / current_price * 100
                                ...
                                logging.info("*********************************************************")
                                logging.info("1. 보유 종목      : " + str(my_item['market']))
                                logging.info("2. 내수익율(%)    : " + str(round(item_income,2)) + " % ")
                                logging.info("3. 매수 금액      : " + str("{0:,}".format(round(item_buy,0))))
                                logging.info("4. 현재 가격      : " + str("{0:,}".format(current_price)))                                
                                logging.info("5. 수익율(현재 %) : " + str(round(current_income,2)) + " % ")
                                logging.info("6. 종가 예상      : " + str("{0:,}".format(predict_price)))                                
                                logging.info("7. 수익율예상(%)  : " + str(round(rev_pcnt,2)) + " % ")
                                logging.info("8. 구매  목표가   : " + str("{0:,}".format(targetPrice)))   
                                logging.info("9. 구매후수익율(%): " + str(round(pre_pcnt,2)) + " % ")                                                           
                                logging.info("*********************************************************") 
                                ...
                                # ----------------------------------------------------------------- 
                                if my_item['market'].split('-')[1] not in sell_except_item : 
                                    if round(rev_pcnt, 2) < loss_cut and round(current_income,2) < 0 :   
