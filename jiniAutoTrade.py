    ...
    try: 
        ...
        while True:
            ...
            if len(my_items) > 0 :
                ...
                item_buy_amt = round(int(tot_present_val/200),-3)
                all_around_amt = int(item_buy_amt*30)

            else :
                # 종목별 매수 금액 설정(일정 금액 직접 입력)
                item_buy_amt = round(int(available_amt/200),-3)   
                all_around_amt  =  available_amt                            
            ...
            logging.info("*********************************************************")
            logging.info("1. 총 보유종목  : " + str(len(my_items))+"개")
            logging.info("2. 현 자산총액  : " + str("{0:,}".format(round(tot_present_val,0))))
            logging.info("3. 현 구매금액  : " + str("{0:,}".format(round(item_buy_amt,0))))
            logging.info("4. 추 매 금 액  : " + str("{0:,}".format(round(add_buy,0))))        
            logging.info("5. 최대구매금액 : " + str("{0:,}".format(round(item_max_buy,0))))      
            logging.info("*********************************************************")   
            ...
            # 올어라운드 매수 시작
            # -----------------------------------------------------------------
            if len(my_items) < 80  and available_amt >= all_around_amt:
